...
We use adaptive restart-refine differential evolution (ARRDE)\cite{muzakka2026robustdifferentialevolutionnonlinear} as the
test optimizer. ARRDE is chosen because it has been evaluated in a study using
CEC benchmark collections from CEC2011, CEC2017, CEC2019, CEC2020, and CEC2022.
That study compared ARRDE against state-of-the-art algorithms and reported that
ARRDE ranked first over the benchmark functions used in the comparison. The
purpose here is not to retune ARRDE, but to use a strong fixed optimizer to show
how the different construction mechanisms change the observed difficulty of the
generated functions.
...
\begin{enumerate}
	...
	\item \textbf{Effect of \(\phi_i\).} We again use the 28 base functions,
		...
		shape.
	\item \textbf{Effect of \(\psi\).} We generate 30 composition instances.
...
construction practical in C++ and Python. The numerical experiment with ARRDE
illustrates how the classification scheme can be used to assess algorithm
robustness across controlled families of landscape mechanisms rather than only
across a small fixed set of manually designed functions.
