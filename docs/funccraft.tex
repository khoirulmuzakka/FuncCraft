...
\author{
	Khoirul Faiq Muzakka and Martin Finsterbusch%
	\thanks{Khoirul Faiq Muzakka and Martin Finsterbusch are with the
	Institute of Energy Materials and Devices (IMD-2), Forschungszentrum
	J\"ulich GmbH, Germany (e-mail: k.muzakka@fz-juelich.de).}
}
...
\begin{abstract}
This paper presents a general framework for constructing scalable continuous
optimization benchmark functions from known base functions. The construction
separates three design choices: coordinate transformations that modify the
search-space geometry, value transformations that reshape individual component
landscapes, and composition functions that combine the transformed components.
For these ingredients, we give sufficient conditions that preserve analytically
known global optima, so new benchmark instances can be generated without
proving each function separately. The same decomposition also gives a natural
classification and metadata scheme for organizing and reproducing generated
instances. A controlled numerical experiment with a differential
evolution-based optimizer illustrates how different choices of coordinate
transformation, value transformation, and composition function affect the
observed optimization difficulty.
\end{abstract}

\begin{IEEEkeywords}
Benchmark functions, derivative-free optimization, evolutionary computation,
...
Consequently, benchmark suites such as the CEC competitions\cite{cec2017, cec2020, cec2022} and the BBOB
testbed\cite{Hansen2009BBOBFunctions} have become standard evaluation platforms for numerical optimization.
...
unrepresented, landscape structures\cite{PIOTROWSKI2023101378}.
...
collection of canonical functions. Consequently, robustness studies may require
...
denoted by \(T_i\), modify the geometry of the search space through operations
such as shifting, rotation, scaling, conditioning, subspace selection, or
nonlinear warping. Second, value transformations, denoted by \(\phi_i\),
reshape the objective values of individual components, allowing the construction
of plateaus, repeated minima, oscillations, saturation, or additional local
minima while preserving known global optima whenever desired. Finally, a
composition function, denoted by \(\psi\), combines the transformed components
into the final objective function, controlling the interaction among component
landscapes, spatial dominance, basin hierarchy, deception, and multimodality.


The same three stages also give a simple way to organize generated
benchmarks. New coordinate transformations, value transformations, or
composition rules can be added without changing the rest of the construction,
so the framework acts more like a design language than a fixed benchmark
suite. Each generated function can then be described by its base functions and
by the classes of \(T_i\), \(\phi_i\), and \(\psi\), together with the
numerical parameters and random seed needed for exact reproduction. This makes
large benchmark collections easier to group, compare, and reproduce.


The framework also establishes general conditions under which the generated
benchmark function retains a known global optimum. These conditions are stated
at the level of the three construction ingredients \(T_i\), \(\phi_i\), and
\(\psi\), rather than separately for every generated function. As a result, a
user only needs to verify that the chosen transformations and composition rule
satisfy the stated conditions; the optimum-preservation guarantee then follows
from the abstract construction. This is useful for optimization benchmarking,
where objective errors, success rates, and convergence behavior must be
evaluated with respect to a known optimum. Beyond preserving known optima, the
framework also provides systematic prescriptions for constructing benchmark
functions with prescribed landscape characteristics, including multimodality,
deceptive basins, nonseparability, ill-conditioning, variable interaction,
repeated optima, and hierarchical compositions.

The primary contribution of this paper is the mathematical and computational
foundation for this construction approach. We establish general
...
ingredients, and show how the same decomposition induces a natural
classification of generated functions. To support practical use, we provide C++
and Python implementations in an open-source package called
\textsc{FuncCraft}, together with a structured labeling and metadata scheme for
organizing large benchmark collections and reproducing individual instances
exactly. We also include a controlled numerical experiment with ARRDE to
illustrate how different choices of coordinate transformation, value
transformation, and composition function affect the observed difficulty of the
generated problems. A broader comparison of many optimization algorithms on
large automatically generated benchmark collections is left for future work.
...
We start at the component level. The next condition ensures that a value
transformation preserves the known zero of a base function and does not create
...
The preceding two propositions control the component values before composition:
\(\phi_i\) preserves nonnegativity and zero values, while \(T_i\) places those
component minima at prescribed points in the full search space. The remaining
condition is imposed on the final composition function \(\psi\).
...
\begin{remark}
	...
	value. Thus, Proposition~\ref{thm:general-minimum-preservation} should be
	interpreted as a practical sufficient condition for constructing benchmark
	functions with a known optimum, not as a complete characterization of all
	possible minimum-preserving compositions.
\end{remark}

Proposition~\ref{thm:general-minimum-preservation} is the general
minimum-preservation statement for the composed function: for any chosen
component functions and any composition rule \(\psi\), it is enough that the
reachable composed values are nonnegative and that the prescribed point is
mapped to zero. The next proposition applies this general statement to a
structured prescribed-point construction. Components are assigned to one or
more prescribed points, the coordinate maps place their component minima there,
and \(\psi\) then determines which prescribed points are global minimizers,
local minimizers, or deceptive local minimizers.
...
\label{sec:anaylsis of benchmark construction}
...
\subsection{\texorpdfstring{Coordinate transformations \(T_i\)}{Coordinate transformations Ti}}
...
\subsection{\texorpdfstring{Value transformations \(\phi_i\)}{Value transformations phi i}}
...
increasing on the relevant range. These choices affect the landscape in
different ways. Linear scaling changes only the vertical scale. A power
transformation changes how quickly the objective grows away from the minimum
value \(u=0\): \(p>1\) makes small positive values even smaller, producing a
flatter basin near the minimizer, whereas \(0<p<1\) makes small positive values
larger, producing a sharper basin. In the latter case, the slope of \(u^p\)
becomes unbounded as \(u\to0^+\), so \(u^p\) is not differentiable at the origin
and the transformed objective may become nonsmooth at the preserved minimizer.
Logarithmic transformations have a different role: they leave the ordering of
objective values unchanged while compressing large values far from the
minimum.
...
\subsection{\texorpdfstring{Composition function \(\psi\)}{Composition function psi}}
...
\[
	f(x)=\psi(x,z(x)).
\]
The composition may depend explicitly on \(x\), for example through spatial
weights or gates. The \(x\)-independent case \(\psi(x,z)=\psi(z)\) remains an
important special case.

By design, \(\psi\) should preserve the known global minimum. For a prescribed
component-value vector \(v^\star=z(x^\star)\), the basic sufficient condition
from Proposition~\ref{thm:general-minimum-preservation} is
\[
	\psi(x^\star,v^\star)=0,
	\qquad
	\psi(x,z(x))\ge0
...
The following subsections turn these general observations into concrete
composition patterns. We first consider the simplest case, where all components
are arranged to share the same prescribed minimizer, and then move to
constructions that deliberately place deceptive local basins at different
points.
...
\begin{proposition}[Common-minimizer specialization]
	\label{prop:common-vanishing-construction}
...
arguments above. However, since the weights $w_i(x)$ are typically distance-based, all unnormalized weights may approach zero when the distances to the prescribed points become large, causing the weighted composition to degenerate toward a trivial value. Normalization prevents this behavior by ensuring that the relative contributions of the component objectives remain nontrivial throughout the domain. If the weights are normalized, then \(\psi(x,z)\) defines a local or spatially varying blend of the shifted component objectives. 
...
components away from these neighborhoods. The figure shows that the prescribed
global minimum remains visible, while an additional local
deceptive minimum appears. Thus the construction can preserve
the known global optimum and, at the same time, introduce a separate attractive
basin that may draw an optimizer away from it. The shape between the two basins
also depends on the selected component pair, illustrating that the weighting
mechanism can create different transition landscapes even when the same
prescribed-point logic is used.
...
The classification follows the same ingredients used to generate the function. A
...
The construction results above allow many valid choices of the composition,
value transformation, and coordinate transformation classes
\((\mathcal{C},\mathcal{P},\mathcal{T})\). Rather than testing all
possible combinations, we use a small controlled experiment to illustrate how
these construction choices affect the difficulty seen by an optimizer. The
experiment uses transformation and composition mechanisms introduced in
Section~\ref{sec:anaylsis of benchmark construction}, so every generated
instance still follows the minimum-preservation conditions established earlier.
The experiment is organized by the structural labels in
Table~\ref{tab:classification-dictionary}: the table gives the general
dictionary used to describe generated functions, while the numerical experiment
below fixes concrete parameter choices for each treatment. This separation is
important because the label identifies the construction mechanism, whereas the
parameter record \(\theta\) determines the particular benchmark instance that an optimization algorithm minimizes.
...
\begin{table*}[t]
	...
	\begin{tabular}{@{}p{0.09\textwidth}p{0.17\textwidth}p{0.30\textwidth}p{0.31\textwidth}@{}}
		\hline
		Class & Label & Meaning & Formula \\
		\hline
		\(\mathcal{C}\) & \texttt{NONE} &
		No composition; single component. &
		\(\psi(x,z)=z_1\) \\
		\(\mathcal{C}\) & \texttt{CPM-SUM} &
		Common-point minimizer; weighted sum. &
		\(\psi(x,z)=\sum_i w_i z_i,\; w_i\ge0\) \\
		\(\mathcal{C}\) & \texttt{CPM-PMEAN} &
		Common-point minimizer; power mean. &
		\(\psi(x,z)=\left(\sum_i \alpha_i z_i^p\right)^{1/p}\) \\
		\(\mathcal{C}\) & \texttt{CPM-LWELL} &
		Common-point minimizer; level wells. &
		\(s=\sum_i z_i,\;\psi=s(1+\epsilon\sin(\alpha s))\) \\
		\(\mathcal{C}\) & \texttt{DPM-SOFTMAX} &
		Deceptive-point minimizer; softmax weights. &
		\(\psi(x,z)=\sum_i w_i(x)(z_i+b_i)\) \\
		\hline
		\(\mathcal{P}\) & \texttt{NONE} &
		No value transformation. &
		\(\phi(u)=u\) \\
		\(\mathcal{P}\) & \texttt{COSZERO} &
		Cosine transformation with repeated zeros. &
		\(\phi(u)=1-\cos(\alpha u)\) \\
		\(\mathcal{P}\) & \texttt{OSC} &
		Oscillatory transformation preserving zero. &
		\(\phi(u)=u(1+\epsilon\sin(\alpha u))\) \\
		\(\mathcal{P}\) & \texttt{POWER} &
		Power transformation controlling flatness. &
		\(\phi(u)=\alpha u^p,\; \alpha\ge0,\;p>0\) \\
		\hline
		...
		\(\mathcal{T}\) & \texttt{ROT} &
		Shifted rotation. &
		\(T_i(x)=R_i(x-\bar x)+x_i^\star\) \\
		\(\mathcal{T}\) & \texttt{BLOCK} &
		Disjoint subspace selection. &
		\(T_i(x)=P_i(x-\bar x)+x_i^\star\) \\
		\(\mathcal{T}\) & \texttt{AFF} &
		General affine map. &
		\(T_i(x)=A_i(x-\bar{x})+x_i^\star\) \\
		\hline
...
belong to \(\theta\). We therefore keep the label fixed when comparing the same
mechanism, and vary \(\theta\) only to generate reproducible instances. 

We use adaptive restart-refine differential evolution
(ARRDE)\cite{muzakka2026robustdifferentialevolutionnonlinear} as the test
optimizer. ARRDE is a strong recent differential-evolution variant that has
been evaluated in an extensive study covering benchmark collections from
CEC2011, CEC2017, CEC2019, CEC2020, and CEC2022. It was compared against
state-of-the-art algorithms and winners of CEC competitions, and was shown to
be very competitive. The purpose here is not to tune ARRDE, but to show how the
different construction mechanisms change the observed difficulty of the
generated functions.

All generated instances are shifted so that the target minimum value is
\(f^\star=100\). Each instance is run 21 times in dimension \(D=10\), using
evaluation budgets \(100D\), \(1000D\), \(3000D\), and \(10000D\). Performance
is measured by the fraction of solved problems and by the transformed relative error
\[
	e_T=\frac{r}{1+r},
	\qquad
	r=\frac{\Delta}{f^\star},
	\qquad
	\Delta=|f_{\mathrm{best}}-f^\star|.
\]
This transformation behaves like the ordinary relative error when the error is
small, but remains bounded by one for very large errors. A problem is counted
as solved if at least half of the 21 runs achieve relative objective error
\(r<10^{-6}\). The solved fraction is then the fraction of benchmark instances
in a treatment that satisfy this condition. The experiment is split into three
controlled parts.

\begin{enumerate}
	\item \textbf{Effect of coordinate transformation \(T_i\).} We start with the 28 base functions and
		apply no value transformation and no composition. Only the coordinate
		transformation is changed. The treatment \texttt{NONE} uses
		\(T_i(x)=x\). The treatment \texttt{ROT} uses
		\(T_i(x)=R_i(x-x^\star)\), where \(R_i\) is a random orthogonal
		matrix and \(x^\star\) is a prescribed minimizer sampled uniformly
		from the domain. The treatment \texttt{AFF} uses
		\(T_i(x)=A_i(x-x^\star)\), where \(A_i\) is generated from a random
		orthogonal matrix with row scales increasing geometrically from
		\(1\) to \(10^2\). Thus this block tests the effect of axis
		alignment, rotation, and affine ill-conditioning.
	\item \textbf{Effect of value transformation \(\phi_i\).} We again use the 28 base functions,
		but now fix the coordinate transformation to \texttt{ROT}. Only the
		value transformation is changed. The treatment \texttt{NONE} uses
		\(\phi(u)=u\). The treatment \texttt{COSZERO} uses
		\(\phi(u)=1-\cos(0.10u)\). The treatment \texttt{OSC} uses
		\(\phi(u)=u(1+0.40\sin(0.03u))\). The power treatment is split into
		two extreme cases: \texttt{POWER\_P01} uses \(\phi(u)=u^{0.1}\),
		which gives a sharp near-minimum shape, while \texttt{POWER\_P10}
		uses \(\phi(u)=u^{10}\), which gives a very flat near-minimum
		shape.
	\item \textbf{Effect of composition function \(\psi\).} We generate 30 composition instances.
		Each instance is formed by randomly selecting \(m=5\) base functions.
		For a fixed instance, the selected base functions, rotations,
		dimension, and value transformation are kept unchanged, and only the
		composition rule is varied. The treatment \texttt{CPM-SUM} uses
		\(\psi(z)=\sum_i z_i\). The treatment \texttt{CPM-PMEAN} uses the
		power mean with exponent \(p=2\). The treatment \texttt{CPM-LWELL}
		uses \(s(1+0.40\sin(0.03s))\), where \(s=\sum_i z_i\). The treatment
		\texttt{DPM-SOFTMAX} uses softmax weights with sharpness \(0.03\),
		local selection radius \(1\), and offsets \(b_i=10i\) for
		\(i=0,\ldots,m-1\). For the common-point minimizer classes, all
		components share the same randomly selected minimizer. For
		\texttt{DPM-SOFTMAX}, the global minimizer is randomly selected from
		the domain, while one deceptive local basin is placed at the origin.
\end{enumerate}
...
\begin{figure*}[t]
	...
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_coord_tre.pdf}
	\end{minipage}
	%\hfill
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_coord_solved_fraction.pdf}
	\end{minipage}
	\caption{Effect of the coordinate transformation \(T_i\) on ARRDE
		performance. Left: average transformed relative error. Right: fraction of
		solved problems.}
	\label{fig:experiment-coordinate}
\end{figure*}
...
\begin{figure*}[t]
	...
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_value_tre.pdf}
	\end{minipage}
%	\hfill
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_value_solved_fraction.pdf}
	\end{minipage}
	\caption{Effect of the value transformation \(\phi_i\) on ARRDE
		performance. Left: average transformed relative error. Right: fraction of
		solved problems.}
	\label{fig:experiment-value}
\end{figure*}
...
\begin{figure*}[t]
	...
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_composition_tre.pdf}
	\end{minipage}
	%\hfill
	\begin{minipage}{0.4\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/experiment_composition_solved_fraction.pdf}
	\end{minipage}
	\caption{Effect of the composition function \(\psi\) on ARRDE performance.
		Left: average transformed relative error. Right: fraction of solved
		problems.}
	\label{fig:experiment-composition}
\end{figure*}


Figure~\ref{fig:experiment-coordinate} shows the effect of the coordinate
transformation \(T_i\) on ARRDE. Both metrics give the same qualitative
ordering: \texttt{NONE} is the easiest treatment, \texttt{ROT} is harder, and
\texttt{AFF} is hardest. This behavior is consistent with the intended role of
the coordinate transformation. The untransformed case preserves the original
axis structure of the base functions. The rotation treatment removes this axis
alignment and makes separable functions nonseparable from the optimizer's point
of view. The affine treatment further changes the scaling of different
directions, so the optimizer must handle both nonseparability and
ill-conditioning. The gap between \texttt{ROT} and \texttt{AFF} becomes smaller
at larger budgets, but \texttt{AFF} remains the more difficult treatment overall.

Figure~\ref{fig:experiment-value} shows that value transformations can change
the interpretation of objective-value success. The \texttt{COSZERO} treatment
appears much easier than the rotated untransformed case. This is expected from
the form \(\phi(u)=1-\cos(0.10u)\), which introduces repeated zero and
near-zero value levels. Consequently, ARRDE can obtain objective values close to
\(f^\star\) without necessarily locating the original prescribed minimizer. The
two power transformations show a different contrast. The \texttt{POWER\_P01}
case gives small transformed relative errors but a low fraction of solved
problems, because \(u^{0.1}\) compresses large objective values but creates a
sharp near-minimum shape. In contrast, \texttt{POWER\_P10} has larger
transformed relative error but a higher solved fraction at larger budgets,
because \(u^{10}\) makes the basin very flat near the minimum while strongly
penalizing points that remain away from it. The \texttt{OSC} treatment is
consistently harder than \texttt{NONE}: it has larger errors and solves fewer
problems at the same budgets, showing that oscillatory value reshaping can
degrade the optimizer's progress.

Figure~\ref{fig:experiment-composition} compares the composition rules. Among
the common-point minimizer constructions, \texttt{CPM-SUM} is the easiest
baseline. The power mean and level-well variants are harder, with
\texttt{CPM-LWELL} producing the largest transformed relative errors among the
common-point compositions. The deceptive softmax treatment behaves differently:
its transformed relative error is not always the largest, but its solved
fraction is the lowest at the larger budgets. This indicates that
\texttt{DPM-SOFTMAX} often allows ARRDE to find good objective values while
still failing to reach the global basin reliably. Thus the deceptive
composition exposes a failure mode that is not captured by average error alone.

These observations suggest practical choices for constructing difficult
benchmark instances. For coordinate transformations, \texttt{ROT} and
\texttt{AFF} are useful when testing sensitivity to nonseparability and
conditioning, with \texttt{AFF} generally being harder. For value
transformations, \texttt{OSC} and the extreme power transformations are more
informative stress tests than \texttt{COSZERO}, because \texttt{COSZERO} can
create repeated near-optimal value levels. For composition functions,
\texttt{DPM-SOFTMAX} is useful when the goal is to test deception and basin
selection, while \texttt{CPM-LWELL} is useful when the goal is to increase
common-minimizer difficulty without moving the global minimizer away from the
shared component minimizer.
...
functions from known base functions. By separating coordinate transformations,
value transformations, and composition functions, the framework makes the main
landscape-design choices explicit while preserving analytically known optima
under simple sufficient conditions. This allows benchmark instances to be
generated systematically without proving each function from scratch.

The same decomposition also provides a natural classification and metadata
scheme based on the selected base functions, transformation classes,
composition class, and instance parameters. This makes generated benchmark
collections easier to organize, reproduce, and analyze. The numerical
experiment with ARRDE illustrates that different choices of these construction
ingredients lead to different observed optimization difficulties, supporting
the use of the framework for controlled studies of algorithm robustness.
...
Grant No. 13XP0445 (For-Analytik). K.F.M., S.M., and M.F.
are supported by this grant.


The authors acknowledge the use of artificial intelligence
tools (ChatGPT, OpenAI) for language refinement and clarity
improvement during manuscript preparation. All technical
content, experiments, and conclusions were developed and
...
\section{Base Function Library}
\label{app:base-functions}

Table~\ref{tab:base-function-library} lists the 28 primitive base functions used
by \textsc{FuncCraft}. The formulas are written in the canonical coordinates
used by the implementation, with \(x=(x_1,\ldots,x_d)\). Some CEC-derived
functions include internal scaling constants; these are included when they are
part of the implemented primitive.

\begingroup
\scriptsize
\setlength{\tabcolsep}{3pt}
\renewcommand{\arraystretch}{1.18}
\begin{longtable}{@{}p{0.20\textwidth}p{0.45\textwidth}p{0.27\textwidth}@{}}
	\caption{Base functions used in the \textsc{FuncCraft} library.}
	\label{tab:base-function-library}\\
	\hline
	Name & Implemented form & Properties \\
	\hline
	\endfirsthead
	\hline
	Name & Implemented form & Properties \\
	...
	\texttt{SumDifferentPowers} &
	\(\sum_i |x_i|^{i}\) &
	Unimodal, separable, heterogeneous coordinate exponents. \\
	\texttt{Ellipsoidal} &
	\(\sum_i 10^{6(i-1)/(d-1)}x_i^2\) &
	Unimodal, separable, ill-conditioned. \\
	\texttt{BuecheRastrigin} &
	\(10d+\sum_i (y_i^2-10\cos(2\pi y_i))\), where
	\(y_i=10x_i\) for positive odd-indexed coordinates and \(y_i=x_i\)
	otherwise &
	Multimodal, separable, asymmetric, highly periodic. \\
	\texttt{LinearSlope} &
	\(\sum_i 100^{(i-1)/(d-1)}|x_i|\) &
	Unimodal, separable, nonsmooth at the optimum. \\
	\texttt{AttractiveSector} &
	\(\left(\sum_i s_i^2x_i^2\right)^{0.9}\), with
	\(s_i=100\) if \(x_i>0\) and \(s_i=1\) otherwise &
	Unimodal, anisotropic; becomes nonseparable under rotation. \\
	\texttt{StepEllipsoidal} &
	\(\sum_i 10^{6(i-1)/(d-1)}y_i^2\), with rounded step variables \(y_i\) &
	Unimodal, nonsmooth, ill-conditioned. \\
	\texttt{StepRastrigin} &
	\(\sum_i (z_i^2-10\cos(2\pi z_i)+10)\), with
	\(z_i=5.12y_i/100\) and stepped \(y_i\) &
	Multimodal, separable, discontinuous. \\
	\texttt{BentCigar} &
	\(x_1^2+10^6\sum_{i=2}^d x_i^2\) &
	Unimodal, ill-conditioned. \\
	\texttt{Discus} &
	\(10^6x_1^2+\sum_{i=2}^d x_i^2\) &
	Unimodal, ill-conditioned. \\
	\texttt{Rosenbrock} &
	\(\sum_{i=1}^{d-1}\left[100(y_i^2-y_{i+1})^2+(y_i-1)^2\right]\),
	\(y_i=2.048x_i/100+1\) &
	Unimodal, nonseparable, narrow curved valley. \\
	\texttt{Ackley} &
	\(-20\exp\!\left(-0.2\sqrt{d^{-1}\sum_i x_i^2}\right)
	-\exp\!\left(d^{-1}\sum_i\cos(2\pi x_i)\right)+20+e\) &
	Multimodal, nonseparable, many local minima. \\
	\texttt{Rastrigin} &
	\(\sum_i (z_i^2-10\cos(2\pi z_i)+10)\), \(z_i=5.12x_i/100\) &
	Multimodal, separable, highly periodic. \\
	\texttt{Griewank} &
	\(1+\sum_i x_i^2/4000-\prod_i\cos(x_i/\sqrt{i})\) &
	Multimodal, nonseparable, oscillatory product term. \\
	\texttt{Schwefel} &
	\(418.982887d-\sum_i z_i\sin(\sqrt{|z_i|})\), with
	\(z_i=10x_i+420.968746\) and boundary corrections &
	Multimodal, separable, deceptive, rugged. \\
	\texttt{SharpRidge} &
	\(x_1^2+100\sqrt{\sum_{i=2}^d x_i^2}\) &
	Unimodal, nonseparable, ridge-shaped valley. \\
	\texttt{DifferentPowers} &
	\(\sqrt{\sum_i |x_i|^{2+4(i-1)/(d-1)}}\) &
	Unimodal, separable, heterogeneous coordinate exponents. \\
	\texttt{Weierstrass} &
	\(\sum_i\sum_{k=0}^{20}0.5^k\cos(2\pi3^k(x_i+0.5))
	-d\sum_{k=0}^{20}0.5^k\cos(2\pi3^k0.5)\) &
	Multimodal, fractal ruggedness; nonseparable under rotation. \\
	\texttt{SchafferF7} &
	\(\left((d-1)^{-1}\sum_{i=1}^{d-1}\sqrt{s_i}
	(1+\sin^2(50s_i^{0.2}))\right)^2\),
	\(s_i=\sqrt{x_i^2+x_{i+1}^2}\) &
	Multimodal, nonseparable, pairwise radial coupling. \\
	\texttt{SchafferF7Cond10} &
	Same as \texttt{SchafferF7}, with coordinate scales based on condition
	\(10\) &
	Multimodal, nonseparable, condition 10. \\
	\texttt{SchafferF7Cond1000} &
	Same as \texttt{SchafferF7}, with coordinate scales based on condition
	\(1000\) &
	Multimodal, nonseparable, condition 1000. \\
	\texttt{GriewankRosenbrock} &
	\(\sum_i\left(z_i^2/4000-\cos z_i+1\right)\), where
	\(z_i=100(y_i^2-y_{i+1})^2+(y_i-1)^2\) and \(y_i=0.05x_i+1\) &
	Multimodal, nonseparable, funnel-like composition. \\
	\texttt{Gallagher101} &
	\(\min_{j=1,\ldots,101} w_j\|x-c_j\|^2\), with deterministic peak
	centers and weights &
	Multimodal, nonseparable, many local optima. \\
	\texttt{Gallagher21} &
	\(\min_{j=1,\ldots,21} w_j\|x-c_j\|^2\), with deterministic peak
	centers and weights &
	Multimodal, nonseparable, few dominant peaks. \\
	\texttt{Katsuura} &
	\(\frac{10}{d^2}\left[\prod_i(1+i a_i)^{10/d^{1.2}}-1\right]\),
	where \(a_i=\sum_{j=1}^{32}|2^jx_i-\mathrm{round}(2^jx_i)|/2^j\) &
	Multimodal, nonseparable, highly rugged. \\
	\texttt{LunacekBiRastrigin} &
	\(\min\{\sum_i(z_i-\mu_1)^2,\ d+s\sum_i(z_i-\mu_2)^2\}
	+10\sum_i(1-\cos(2\pi z_i))\), with \(z_i=0.2x_i+\mu_1\) &
	Multimodal, deceptive, double-funnel landscape. \\
	\texttt{Zakharov} &
	\(\sum_i x_i^2+\left(\sum_i 0.5ix_i\right)^2
	+\left(\sum_i 0.5ix_i\right)^4\) &
	Unimodal, nonseparable, polynomial coupling. \\
	\texttt{Levy} &
	\(\sin^2(\pi w_1)+\sum_{i=1}^{d-1}(w_i-1)^2
	(1+10\sin^2(\pi w_i+1))+(w_d-1)^2(1+\sin^2(2\pi w_d))\),
	\(w_i=1+x_i/4\) &
	Multimodal, nonseparable, periodic structure. \\
\end{longtable}
\endgroup
